...
ABL wind over city blocks set up for OpenFOAM v9. Geometry lives in `geometry/city_buildings.stl`; `snappyHexMesh` expects the `city_buildings.eMesh` edges generated by `surfaceFeatures` (see run steps below).
...
```bash
surfaceFeatures                            # writes geometry/city_buildings.eMesh
blockMesh
...
- `snappyHexMeshDict` points to `geometry/city_buildings.stl` and the generated `geometry/city_buildings.eMesh`; `surfaceFeaturesDict` is included to build that edge file.
