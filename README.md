...
ABL wind over city blocks set up for OpenFOAM v9. Geometry lives in `constant/geometry/city_buildings.stl`; `snappyHexMesh` expects the `city_buildings.eMesh` edges generated by `surfaceFeatureExtract` (see run steps below).
...
```bash
surfaceFeatureExtract                      # writes constant/geometry/city_buildings.eMesh
blockMesh
...
- `snappyHexMeshDict` points to `constant/geometry/city_buildings.stl` and the generated `constant/geometry/city_buildings.eMesh`; `surfaceFeatureExtractDict` is included to build that edge file.
